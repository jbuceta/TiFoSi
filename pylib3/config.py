...
if len(dglobal['itissue']['file']) == 0:
  ...
  comprova_propietats_tag(temp, dict(), list(dglobal['types'].keys()))
...
for p in temp:
  if 'stage' in p.attrib:
    ...
    escriuError(["A <potential> tag have not properly set the 'stage' attribute. The defined attributes are:", list(p.attrib.keys())])
  if stemp == "all":
    rangetemp = list(range(1,len(stages)+1))
  else:
    if stemp in list(stages.keys()):
      rangetemp = list(range(int(stemp),int(stemp)+1))
    else:
      escriuError(["The value of 'stage' attribute of a <potential> tag is not valid. The valid attributes are:", list(stages.keys()).append('all')])

  for s in rangetemp:
    for c in p:
      if c.tag not in dictConstants:
        escriuError(["'" + c.tag + "' is not a valid constant. The valid constants are:", list(dictConstants.keys())])
...
for c in eCycles:
  if 'stage' in c.attrib:
    ...
    escriuError(["A <cycle> tag have not properly set the 'stage' attribute. The defined attributes are:", list(c.attrib.keys())])
  if stemp == "all":
    rangetemp = list(range(1,len(stages)+1))
  else:
    if stemp in list(stages.keys()):
  ...
  for att in c:
    ...
    if att.attrib['t'] not in dglobal['types']:
      escriuError(["The attribute 't' of tag <"+att.tag+"> defines a non valid cell type: '"+att.attrib['t']+"'. Valid types are:",list(dglobal['types'].keys())])
...
for p in temp:
  ...
  for ic in ictemp:
    ...
    if ic.attrib['t'] not in proteines[p.attrib['species']]['inicial']:
      escriuError(["A initial condition <"+ic.tag+"> has defined an invalid attribute 't'. Valid attributes are: ",list(proteines[p.attrib['species']]['inicial'].keys())])
    try:
